...
website_data <- website_data %>%
  mutate(across(all_of(rating_cols), ~ parse_number(as.character(.))))
website_data <- website_data %>%
  mutate(across(all_of(user_cols), ~ parse_number(as.character(.))))
...
#Inital Rating is the first ratings just using the 3 #sources, the others keep their name from earlier
...
This gives us a better snapshot into how thinsg played out and visualizes some outliars. We can see that shows like Bluey and Dekalog outpreformed a lot in the user_rating, likely due to them having a lot more votes, and a higher rating on IMDB than any other website.
